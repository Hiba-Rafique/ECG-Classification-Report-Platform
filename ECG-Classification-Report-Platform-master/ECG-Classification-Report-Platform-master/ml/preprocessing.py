...
def load_ecg_file(file_path: str) -> Tuple[np.ndarray, int]:
    ...
    """
    signal, fs, _ = load_ecg_file_with_meta(file_path)
    return signal, fs


def load_ecg_file_with_meta(file_path: str) -> Tuple[np.ndarray, int, List[str]]:
    """
    Load an ECG file and return (signal, sampling_rate, lead_names).

    Same loaders as load_ecg_file, but also reports the lead names the file
    itself carries (WFDB signal names, EDF labels, CSV headers), falling
    back to the standard PTB-XL order when none are recognizable.
    """
...
# Standard PTB-XL lead order (the order the WFDB headers use)
DEFAULT_LEAD_NAMES = [
...
_CANONICAL_LEADS = {
    ...
    "V1": "V1", "V2": "V2", "V3": "V3",
    "V4": "V4", "V5": "V5", "V6": "V6",
    "MLI": "I", "MII": "II", "MIII": "III",
}


def _canonical_lead(name: str) -> str:
    """Normalize a lead label ('AVR', 'ecg avf', 'V-4'…) to display form."""
    key = str(name).strip().upper().replace(" ", "").replace("-", "")
    if key in _CANONICAL_LEADS:
        return _CANONICAL_LEADS[key]
    # Substring fallback for labels like 'ECG AVF' or 'Lead V2'
    for token, canonical in (
        ("III", "III"), ("II", "II"), ("AVR", "aVR"),
        ("AVL", "aVL"), ("AVF", "aVF"),
        ("V6", "V6"), ("V5", "V5"), ("V4", "V4"),
        ("V3", "V3"), ("V2", "V2"), ("V1", "V1"),
        ("I", "I"),
...
def _csv_lead_names(columns) -> List[str]:
    """Use CSV column headers when they are recognizable lead names."""
    canonical = [_canonical_lead(str(c)) for c in columns]
    if len(canonical) == 12 and set(canonical) == set(DEFAULT_LEAD_NAMES):
        return canonical
    return list(DEFAULT_LEAD_NAMES)


def _load_csv(file_path: str) -> Tuple[np.ndarray, int, List[str]]:
    ...
    fs = 500
    return signal, fs, _csv_lead_names(df.columns)


def _load_wfdb(file_path: str) -> Tuple[np.ndarray, int, List[str]]:
    ...
    lead_names = [_canonical_lead(n) for n in (record.sig_name or [])]
    if len(lead_names) != record.n_sig:
        lead_names = list(DEFAULT_LEAD_NAMES)
    return record.p_signal, record.fs, lead_names


def _load_edf(file_path: str) -> Tuple[np.ndarray, int, List[str]]:
    ...
    try:
        ...
        try:
            labels = [str(lab) for lab in f.getSignalLabels()]
        except Exception:
            labels = []
    finally:
        f.close()
    ...
    lead_names = [_canonical_lead(n) for n in labels]
